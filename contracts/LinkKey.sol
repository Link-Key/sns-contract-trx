...
contract LinkKey is ERC20Upgradeable,OwnableUpgradeable,PausableUpgradeable {
    ...
    TimersUpgradeable.Timestamp public releaseTime;
    ...
    function initialize(string memory name_, string memory symbol_,
        ...
        uint256 releaseAmount_) public initializer {
    ...
    function mint() public returns(bool){
        ...
        uint256 userCollectToken = releaseAmount.mul(1 trx).mul(userShare).div(totalShare);
        uint256 teamCollectToken = releaseAmount.mul(1 trx).mul(teamShare).div(totalShare);
        uint256 investorCollectToken = releaseAmount.mul(1 trx).mul(investorShare).div(totalShare);
        uint256 bidderCollectToken = releaseAmount.mul(1 trx).mul(bidderShare).div(totalShare);
